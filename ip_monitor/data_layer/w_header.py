...
class WirelessHeader(Structure):
    ...
    def __get_header_length(self):
        length = 24
        if (self.frame_type == 'Data' and
            (self.subtype == 'QoS' or self.subtype == 'NullQos')):
            length += 2

        # check if header uses address4
        if self.DSstatus == 3:
            length += 6
    ...
    def __init__(self, socket_buffer = None):
        ...
        self.protected = self.frame_control[1] >> 6 & 1
        self.DSstatus = self.frame_control[1] & 3
        ...
        self.length = self.__get_header_length()
